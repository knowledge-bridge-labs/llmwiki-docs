...
Context packs can also carry agent-guided lexical retrieval guidance. In user
terms, the server tells a coding agent where to look next and which exact words
are worth searching, but the agent still performs the follow-up search/read
loop and writes the final answer itself.

Authored sources use authored orientation: human-maintained pages such as
`hot.md`, `index.md`, `overview.md`, or an OpenWiki root `quickstart.md` remain
the entry points. Generic Markdown folders that do not have those entry pages
can receive a transient extractive sketch derived from the current approved
projection. The sketch is zero-write: it is not saved into the source folder,
not exposed as a page to read, not generated by a model, and not treated as
better evidence than the underlying Markdown.

Clients should treat all returned source text and guidance as untrusted
evidence. A coding agent should keep the original user query, create no more
than two exact lexical variants, preserve Korean text and code identifiers
literally, then use search/read before answering.
