...
| 4 | Add reproducible screenshot capture workflow. | Pending | Replace one-off Playwright capture specs with a maintained command such as `npm run docs:capture-screenshots`. |
| 5 | Decide `llmwiki-agent-bridge` `compact-json` default promotion path. | Pending | Loop 21 approved `compact-json` for the candidate matrix, but broad production default needs multi-runtime/model approval evidence and docs/ADR update. |
