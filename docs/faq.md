...
development scripts. Use the published `llmwiki-serve==0.2.11` package when you
...
current public-preview baseline: `llmwiki-serve==0.2.11`,
...
`uvx --from llmwiki-serve==0.2.11 llmwiki-serve ...` for the source server. Use
...
`npm exec --package llmwiki-agent-bridge@0.6.0 -- llmwiki-agent-bridge` when
