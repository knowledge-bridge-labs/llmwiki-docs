...
`llmwiki-serve==0.2.6` is the current source-server package baseline.
...
Public registry checks confirmed `llmwiki-serve 0.2.6`,
...
[Evidence](/evidence). The current SciFact rows describe a source candidate for
the next `llmwiki-serve` release, not the live PyPI baseline. They are not
vendor-runtime certification, model-answer quality certification, or an
official BEIR leaderboard submission.

Publication evidence:

| Package | Current version | Evidence |
| --- | --- | --- |
| `llmwiki-serve` | `0.2.6` | PyPI latest verified; keep future candidate docs separate until PyPI publish verification completes. |
...
| PyPI/npm packages | Published | Use package-manager installs for `llmwiki-serve==0.2.6`, `llmwiki-bridge-start@0.0.3`, `llmwiki-agent-bridge@0.3.0`, and the static `llmwiki-chat@0.1.6` artifact. |
...
| --- | --- | --- | --- | --- | --- |
| `llmwiki-serve` | Python package 0.2.6, Apache-2.0, CLI entrypoint | PyPI published 0.2.6 | Package install or source checkout with `uv sync --extra dev` | Python 3.11+ | `uv run python scripts/release_smoke.py` |
...
`llmwiki-serve` 0.2.6 includes local `ls`/`status` discovery. It combines the
...
The next `llmwiki-serve` source candidate adds immutable BEIR SciFact benchmark
reports on the public feature branch, with exact Windows/DGX quality parity.
Keep package install docs at `0.2.6` until the PyPI release is completed and
verified.
